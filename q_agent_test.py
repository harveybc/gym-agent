# agent_svm:    Load a dataset generated by q-datagen for reading the reward per 
...
class QAgent():    
    ...
    def __init__(self):
        ...
        register(
            ...
            kwargs={'dataset': self.env_f ,'volume':0.1, 'sl':2000, 'tp':2000,'obsticks':self.obsticks, 'capital':100000, 'leverage':100}
        )
    ...
    ## Select the action from the test model (table) with biggest predicted reward
    ## For this, search the exact observation, only the first tick of each observation 
    ## is required to compare during search.
    ## After finding the observation, compare the 4 action signals and select 
    ## the highest as next action. 
